...
Run `npm install` once, then use `npm run dev` for an interactive CLI session (tsx + Node ≥20.19.0). `npm run build` runs `tsdown` for the bundle and `tsc --emitDeclarationOnly` for types, emitting to `dist/cli/index.cjs` and `dist/*.d.ts`; tsdown cleans its output automatically. Quality gates: `npm run lint` (Biome v2 lint), `npm run format` / `npm run format:check` (Biome formatter), `npm run check` (lint + `tsc --noEmit`), and `npm run typecheck`. Execute `npm run test`, `npm run test:watch`, or `npm run test:coverage` via Vitest depending on the TDD phase. CI mirrors the npm commands and uses `npm pack --pack-destination tmp-toolchain/` to capture the distributable tarball.
